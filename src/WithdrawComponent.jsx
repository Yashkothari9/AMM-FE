...
export default function WithdrawComponent( props ){
    ...
    const onChangeAmountOfShare = async (e) => {
        ...
        if(!['', '.'].includes(e.target.value) && props.contract !== null) { 
            try {
                let response = await props.contract.getWithdrawEstimate(e.target.value * PRECISION);
                setEstimateTokens([response.amountToken1 / PRECISION, response.amountToken2 / PRECISION]);
            } catch (err) {
    ...
    const getMaxShare = async () => {
        if(props.contract !== null) {
            ...
            let response = await props.contract.getWithdrawEstimate(props.maxShare * PRECISION);
            setEstimateTokens([response.amountToken1 / PRECISION, response.amountToken2 / PRECISION]);
        } else
            ...
            {   estimateTokens.length > 0 &&
                <div className = "withdrawEstimate">
                    <div className = "amount">
                        Amount of Kar: {estimateTokens[0]}
                    </div>
                    <div className = "amount">
                        Amount of Kothi: {estimateTokens[1]}
                    </div>
                </div>
            }
            <div className ="bottomDiv">
                <div  className ="btn" onClick = {() => withdrawShare()}>Withdraw</div>
            </div>
        </div>
